...
from configs import files, dirs
...
class DataManager:
    ...
    def __init__(self, configs, logger):
        ...
        self.train_file = files.TRAIN_FILE
    ...
    def load_vocab(self):
        ...
        if not self.configs.use_bert:
            with open(self.token2id_file, 'r', encoding='utf-8') as infile:
                for row in infile:
                    row = row.strip()
                    token, token_id = row.split('\t')[0], int(
                        row.split('\t')[1])
    ...
    def padding(self, sample):
        ...
        for i in range(len(sample)):
            if len(sample[i]) < self.max_sequence_length:
                sample[i] += [
                    self.token2id[self.PADDING]
                    for _ in range(self.max_sequence_length - len(sample[i]))
                ]
        return sample
        ...
        for index, record in tqdm(df.iterrows()):
            ...
            if str(token) == str(np.nan):
                if len(tmp_x) <= self.max_sequence_length - 2:
                    ...
                    # padding
                    tmp_x += [
                        0 for _ in range(self.max_sequence_length - len(tmp_x))
                    ]
                    tmp_y += [
                        self.label2id[self.PADDING]
                        for _ in range(self.max_sequence_length - len(tmp_y))
                    ]
                    tmp_att_mask += [
                        0 for _ in range(self.max_sequence_length -
                                         len(tmp_att_mask))
                    ]
                    X.append(tmp_x)
                    y.append(tmp_y)
    ...
    def prepare_single_sentence(self, sentence):
        ...
        if self.configs.use_bert:
            if len(sentence) <= self.max_sequence_length - 2:
                ...
                att_mask += [
                    0 for _ in range(self.max_sequence_length - len(att_mask))
                ]
            else:
            ...
            if len(x) < self.max_sequence_length:
                sentence += [
                    '[PAD]'
                    for _ in range(self.max_sequence_length - len(sentence))
                ]
                x += [
                    self.token2id[self.PADDING]
                    for _ in range(self.max_sequence_length - len(x))
                ]
