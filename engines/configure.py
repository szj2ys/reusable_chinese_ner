...
class Configure:
        ...
        for line in fins:
            ...
            if '=' in line:
                ...
                try:
                    if item in config:
                        print(
                            'Warning: duplicated config item found: {}, updated.'
                            .format((pair[0])))
                    ...
                    print(
                        'configuration parsing error, please check correctness of the config file.'
                    )
                    exit(1)
        return config
    ...
    def show_data_summary(self, logger):
        ...
        logger.info('     datasets         fold: {}'.format(
            self.datasets_fold))
        ...
        logger.info('     checkpoints       dir: {}'.format(
        ...
        logger.info('     measuring     metrics: {}'.format(
            self.measuring_metrics))
        logger.info(' ' + '++' * 20)
        ...
        logger.info('     CUDA  VISIBLE  DEVICE: {}'.format(
        ...
        logger.info('     max       checkpoints: {}'.format(
            self.checkpoints_max_to_keep))
        logger.info('     print       per_batch: {}'.format(
            self.print_per_batch))
